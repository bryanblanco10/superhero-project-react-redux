...
import { checkIfUserIsAuth, submitLogin } from "../../redux/actions/auth";
...
export default function Login() {
  ...
  // const { isCheckingAuth, isAuth } = useSelector((state) => state.authReducer)
  ...
  }, [isSuccessLogged, history])

  const handleSubmitForm = async (evt) => {
    evt.preventDefault();

    if (name?.length && email?.length) {
      await dispatch(submitLogin(name, email));
    }
  };
